 ...
 * a real number of authorization URLs built or ID tokens decoded -- never on install, never on a timer.
...
const KEY_HIT_COUNT = 'oauth2OidcFlowCompanion.review.hitCount';
const KEY_ANSWERED = 'oauth2OidcFlowCompanion.review.answered';
...
/**
 * Call this once per real authorization URLs built or ID tokens decoded. Safe to call multiple times; a
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `OAuth2/OIDC Flow Companion: you've used this ${HITS_BEFORE_PROMPT} times -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
